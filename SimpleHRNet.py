...
from models_.detectors.yolox.yolox.exp import get_exp
from models_.detectors.yolox.yolox.utils import cxcywh2xyxy
...
class SimpleHRNet:
    ...
    def __init__(self,
        ...
        if self.multiperson:
            ...
            elif self.yolo_version == 'x':
                from models_.detectors.YOLOX import YOLOX
            else:
        ...
        if not self.multiperson:
            ...
            if self.yolo_version == 'v3':
                ...
                exp = get_exp('models_/detectors/yolox/exps/default/yolox_x.py', 'yolox_x')
                self.detector = YOLOX(exp, device=device)
            self.transform = transforms.Compose([
    ...
    def _predict_single(self, image):
        def is_bbox_valid(bbox, img_size):
            ...
            """
            x1, y1, x2, y2 = bbox
            height, width, c = img_size

            if x1 < 0 or x2 > width - 1:
                return False
            if y1 < 0 or y2 > height - 1:
                return False
            ...
            if detections is not None:
                for i, (x1, y1, x2, y2, conf, cls_conf, cls_pred) in enumerate(detections):
                    ...
                    if correction_factor > 1:
                        ...
                        y2_new = y2
                        pad_tuple = None
                    # length = x2_new-x1_new
                    # x1_new = int(x1_new - length // 2)
                    # x2_new = int(x2_new - length // 2)
                    # length = y2_new-y1_new
                    # y1_new = int(y1_new - length // 2)
                    # y2_new = int(y2_new - length // 2)
                    #
                    # h, w, c = image.shape
                    ...
                    #
                    # if x1_new > w-1 : x1_new = w-1
                    # if x2_new > w-1 : x2_new = w-1
                    # if y1_new > h-1 : y1_new = h-1
                    # if y2_new > h-1 : y2_new = h-1
